--Step 1: Performing Indexing

-- Index on trans_dt for date filtering
CREATE INDEX idx_trans_dt 
ON `${truecall_src_project_id}.${truecall_src_dataset_name}.${truecall_source_tblname}` (trans_dt);

-- Composite index on arrival and arrival_dt_hr
CREATE INDEX idx_arrival_arrival_dt_hr 
ON `${truecall_src_project_id}.${truecall_src_dataset_name}.${truecall_source_tblname}` (arrival, arrival_dt_hr);

-- Index on end_enb for optimizing lookup or joins
CREATE INDEX idx_end_enb 
ON `${truecall_src_project_id}.${truecall_src_dataset_name}.${truecall_source_tblname}` (end_enb);

-- Index on start_enb for optimizing lookup or joins
CREATE INDEX idx_start_enb 
ON `${truecall_src_project_id}.${truecall_src_dataset_name}.${truecall_source_tblname}` (start_enb);

-- Step 2: Preprocessing data not to use functions in WHERE clauses
...
-- Step 3: Use precomputed values in WHERE clause and batch inserts
